package com.licwim.shaauth.configs;

import com.licwim.shaauth.ShaAuth;
...
public class Config {
    ...
    public static void init() {
        // Getting the plugin.
        Plugin plugin = Bukkit.getServer().getPluginManager().getPlugin(ShaAuth.PluginName);

        if (plugin == null) {
            // Something goes wrong, log it.
            Bukkit.getLogger().log(
                    Level.WARNING,
                    MessageFormat.format("Cannot get plugin {0}", ShaAuth.PluginName)
            );
            return;
        }
